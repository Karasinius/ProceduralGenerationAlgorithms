...
public class RecursiveDivisionGenerator : MonoBehaviour
{
    ...
    private bool ValidateSetup()
    {
        ...
        if (mapWidth % 2 == 0)
        {
            Debug.Log("[RecursiveDivisionGenerator] mapWidth is even Ч decreasing by 1 to make it odd for cell mapping.");
            mapWidth = Math.Max(3, mapWidth - 1);
        }
        if (mapHeight % 2 == 0)
        {
            Debug.Log("[RecursiveDivisionGenerator] mapHeight is even Ч decreasing by 1 to make it odd for cell mapping.");
